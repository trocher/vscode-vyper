...
import vyper.builtins
import vyper.builtins.functions
import vyper.semantics
import vyper.semantics.types


class VyperLang:
    # https://github.com/ethereum/vyper/blob/master/vyper/functions/functions.py
    builtin_functions = vyper.builtins.functions.DISPATCH_TABLE.keys()
    builtin_raw_functions = vyper.builtins.functions.STMT_DISPATCH_TABLE.keys()
    # https://github.com/ethereum/vyper/blob/master/vyper/utils.py
    # base_types = set([]) # types = vyper.semantics.types.get_types().keys()
    base_types = set(
        [
            x
            for x in vyper.semantics.types.PRIMITIVE_TYPES.keys()
            if not x.startswith("$")
        ]
    )  # types = vyper.semantics.types.get_types().keys()
    units = set(
        [
            "ether",
            "wei",
    ...
    )  # this might replace global_keywords, units, call_keywords. Investigate /DBIT
